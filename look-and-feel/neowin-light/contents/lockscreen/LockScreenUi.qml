...
/*
    NeoWin note: This is a copy of Plasma's stock LockScreenUi.qml (from
    plasma-desktop) with a minimal patch to force light textColor on
    Complementary. This ensures the clock (and other lockscreen text) remains
    readable (white on dark) regardless of whether the active global theme is
    neowin-light or neowin-dark.

    When updating Plasma, compare this file against the new stock version in
    /usr/share/plasma/shells/org.kde.plasma.desktop/contents/lockscreen/
    and re-apply the ~8 line patch around the Kirigami.Theme.colorSet block
    if the surrounding code hasn't changed significantly.

    The other .qml files in this dir are also copied snapshots for a
    self-contained LAF-provided lockscreen.
*/
...
Item {
    ...
    // NeoWin addition: force light text on the lockscreen clock & UI elements.
    // The stock lockscreen (and breeze Clock component) sometimes ends up with
    // dark text even when Complementary ForegroundNormal is white (observed with
    // custom light LAFs / after lookandfeel switch). The blurred wallpaper is
    // always dark, so we hard-require light text here for readability in both
    // light and dark modes. Matches NeoWin's Complementary (42,46,50 bg / white fg).
    Kirigami.Theme.textColor: "#fcfcfc"
    Kirigami.Theme.backgroundColor: "#2a2e32"
